...
esp_err_t tinyusb_msc_storage_deinit_spiflash(void) {
    if (s_storage_handle == NULL) {
        ESP_LOGW(TAG, "Storage already deinitialized");
        return ESP_OK;
    }

    // 这里可以添加任何特定于 SPI Flash 的反初始化步骤
    // 例如，卸载 wear leveling 等

    // 卸载 wear leveling
    if (s_storage_handle->wl_handle != WL_INVALID_HANDLE) {
        esp_err_t ret = wl_unmount(s_storage_handle->wl_handle);
        if (ret != ESP_OK) {
            ESP_LOGE(TAG, "Failed to unmount wear leveling");
            return ret;
        }
        ESP_LOGI(TAG, "Wear leveling unmounted successfully");
    }
